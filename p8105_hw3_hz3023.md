...
![](p8105_hw3_hz3023_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->

The table shows that most people in this group have more than high
school education. Other than female on the high school equivalent group,
most young people trend to have a higher degree.

``` r
nhanes |>
  mutate(totalactivity = rowSums(across(starts_with("min")), na.rm = TRUE)) |>
  group_by(totalactivity, seqn, sex, education, age) |>
  ggplot(aes(x = age, y = totalactivity, color = sex)) +
  geom_point() +
  geom_smooth(se = FALSE) +
  facet_grid(.~education) +
  labs(title = "Total Daily Activity and Age in different Sex",
    x = "Age",
    y = "Total Activity",
    color = "Sex") +
  theme_minimal() +
...
![](p8105_hw3_hz3023_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->

Plots shows that the total activity decreases with age in all education
levels. However, in higher education level, the decline is relatively
slow. This happens because compare with lower education levels groups,
people who have more than high school education have a lower total
activity rate. People with a higher level of education tend to avoid
jobs that mainly involve physical labor.
...
nhanes |>
  ...
  geom_smooth(se = FALSE) +
  facet_grid(.~ education) +
  labs(title = "24-hour activity and Education in different Sex",
       x = "Hour", y = "Mean Activity", color = "Sex") +
  theme_minimal() +
  theme(legend.position = "bottom")
```
...
![](p8105_hw3_hz3023_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->

The pattern for activity hour have the same trend in both sex and all
education level over all. People activity start at 5am and peak around
10. Overall, men have more activity than women. Like last plot, people
who are less than high school level have the highest mean activity,
because of the type of job.
